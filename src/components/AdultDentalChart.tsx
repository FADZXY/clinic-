...
function buildTeeth(): ToothConfig[] {
  ...
  for (let i = 0; i < 8; i++) {
    result.push({ num: 11 + i, x: centerX - spacing * (0.5 + i), y: baseY_up + i * archDrop });
  }
  for (let i = 0; i < 8; i++) {
    result.push({ num: 21 + i, x: centerX + spacing * (0.5 + i), y: baseY_up + i * archDrop });
  }
  for (let i = 0; i < 8; i++) {
    result.push({ num: 41 + i, x: centerX - spacing * (0.5 + i), y: baseY_low - i * archDrop });
  }
  for (let i = 0; i < 8; i++) {
    result.push({ num: 31 + i, x: centerX + spacing * (0.5 + i), y: baseY_low - i * archDrop });
  }
...
interface AdultDentalChartProps {
...
export default function AdultDentalChart({
  onToothSelect,
  onHover,
  selectedTooth,
  toothColors,
  ...
  return (
    <svg
      ...
      className={`w-full h-auto max-w-3xl mx-auto ${className}`}
      preserveAspectRatio="xMidYMid meet"
    >
      <defs>
        <filter id="tooth-shadow" x="-10%" y="-10%" width="130%" height="130%">
        ...
        return (
          ...
          >
            ...
            <path
              ...
              strokeLinecap="round"
              filter={isSelected ? "url(#tooth-shadow)" : undefined}
      ...
      <text x={500} y={60} textAnchor="middle" className="fill-sky-600 text-[13px] select-none" style={{ fontFamily: "Tajawal, sans-serif", fontWeight: 700 }}>
        الفك العلوي
      </text>
      <text x={500} y={500} textAnchor="middle" className="fill-sky-600 text-[13px] select-none" style={{ fontFamily: "Tajawal, sans-serif", fontWeight: 700 }}>
        الفك السفلي
      </text>
    </svg>
  );
