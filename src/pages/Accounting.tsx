...
interface FilteredPayment {
  ...
  paymentCount: number; totalPaid: number; totalTreatment: number;
}
...
export default function Accounting() {
  ...
  const dateResults = useMemo<FilteredPayment[]>(() => {
    ...
    for (const patient of patients) {
      ...
      for (const row of patient.treatments) {
        ...
        cnt++; paid += p; treat += parseAmount(row.treatmentAmount);
      }
      if (cnt > 0) out.push({ patientId: patient.id, patientName: patient.name, paymentCount: cnt, totalPaid: paid, totalTreatment: treat });
    }
  ...
  const totalIncome    = incomeResults.reduce((s, r) => s + r.totalPaid,      0);
  const totalTreatment = incomeResults.reduce((s, r) => s + r.totalTreatment, 0);
  const totalRemaining = totalTreatment - totalIncome;
  ...
  function openAddForm() {
    setEditingExp(null);
    setNewDesc(""); setNewAmount(""); setNewCategory("أخرى"); setNewDate(todayStr);
    setShowForm(true);
  }
  ...
  function handlePrint() {
    ...
    const incRows = incomeResults.map((r) =>
      `<tr><td>${formatId(r.patientId)}</td><td>${r.patientName}</td><td>${r.paymentCount}</td><td style="font-weight:bold;color:#10b981">${r.totalPaid.toLocaleString("en-US")} ل.س</td></tr>`
    ).join("");
    const expRows = filteredExpenses.map((e) =>
      `<tr><td>${formatDateAr(e.date)}</td><td>${e.description}</td><td>${e.category}</td><td style="font-weight:bold;color:#ef4444">${e.amount.toLocaleString("en-US")} ل.س</td></tr>`
    ).join("");
...
<div class="cards">
  <div class="card c-green"><p>الإيرادات</p><div class="val" style="color:#10b981">${totalIncome.toLocaleString("en-US")} ل.س</div></div>
  <div class="card c-red"><p>المصاريف</p><div class="val" style="color:#ef4444">${totalExpenses.toLocaleString("en-US")} ل.س</div></div>
  <div class="card c-blue"><p>صافي الربح</p><div class="val" style="color:${netProfit >= 0 ? "#0ea5e9" : "#ef4444"}">${netProfit.toLocaleString("en-US")} ل.س</div></div>
  <div class="card c-amber"><p>متبقي العلاجات</p><div class="val" style="color:#d97706">${totalRemaining.toLocaleString("en-US")} ل.س</div></div>
</div>
      ...
      <div className="pt-20 max-w-4xl mx-auto px-4 pb-12">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6">
          <div className="bg-white rounded-2xl border-2 border-emerald-100 p-4 text-center">
            ...
            <p className="text-lg font-bold text-emerald-600">{totalIncome.toLocaleString("en-US")}<span className="text-xs font-normal text-gray-400 mr-1">ل.س</span></p>
          </div>
          <div className="bg-white rounded-2xl border-2 border-red-100 p-4 text-center">
            ...
            <p className="text-lg font-bold text-red-500">{totalExpenses.toLocaleString("en-US")}<span className="text-xs font-normal text-gray-400 mr-1">ل.س</span></p>
          </div>
          <div className={`bg-white rounded-2xl border-2 p-4 text-center ${netProfit>=0?"border-sky-100":"border-red-200"}`}>
            <p className="text-xs font-semibold text-gray-500 mb-1">صافي الربح</p>
            <p className={`text-lg font-bold ${netProfit>=0?"text-sky-700":"text-red-600"}`}>
              {netProfit<0&&<span className="text-sm">-</span>}{Math.abs(netProfit).toLocaleString("en-US")}<span className="text-xs font-normal text-gray-400 mr-1">ل.س</span>
            </p>
          </div>
          <div className="bg-white rounded-2xl border-2 border-amber-100 p-4 text-center">
            ...
            <p className="text-lg font-bold text-amber-600">{totalRemaining.toLocaleString("en-US")}<span className="text-xs font-normal text-gray-400 mr-1">ل.س</span></p>
          </div>
        </div>
                  ...
                  {incomeResults.map((r)=>(
                    <div key={r.patientId} onClick={()=>openPatient(r.patientId)}
                      ...
                      <div className="text-left flex-shrink-0">
                        <p className="text-sm font-bold text-emerald-600">{r.totalPaid.toLocaleString("en-US")} ل.س</p>
            ...
            {showForm && (
              <div className="bg-white rounded-2xl border-2 border-red-100 p-5 mb-5 shadow-sm">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
                  ...
                  <div>
                    <label className="text-xs font-semibold text-gray-500 block mb-1.5">المبلغ (ل.س) *</label>
              ...
              <div className="space-y-2">
                {filteredExpenses.map((exp)=>(
                  <div key={exp.id} className="flex items-center gap-4 p-4 bg-white rounded-2xl border-2 border-gray-100">
                    ...
                    <div className="text-left flex-shrink-0">
                      <p className="text-sm font-bold text-red-500">{exp.amount.toLocaleString("en-US")} ل.س</p>
                    </div>
                    {/* تعديل */}
