...
function buildActivity(patients: Patient[], appointments: Appointment[], expenses: Expense[]): ActivityItem[] {
  ...
  for (const e of expenses) {
    ...
    items.push({
      ...
      type: "expense",
      title: e.description,
      subtitle: `${e.category} • ${e.currency === "USD" ? "$" : ""}${e.amount.toLocaleString("en-US")}${e.currency !== "USD" ? " ل.س" : ""}`,
      badge: "مصروف",
...
export default function Statistics() {
  ...
  // ─── حسابات الإيرادات الشهرية (USD / SYP) ───
  ...
  // ─── حسابات المصاريف الشهرية (USD / SYP) ───
  const monthlyExpensesSYP = useMemo(() => {
    const arr = Array(12).fill(0);
    for (const e of expenses) {
  ...
  // ─── مجاميع السنة (ثنائي العملة) ───
  const yearIncomeSYP   = monthlyIncomeSYP.reduce((s, v) => s + v, 0);
  const yearIncomeUSD   = monthlyIncomeUSD.reduce((s, v) => s + v, 0);
  const yearExpensesSYP = monthlyExpensesSYP.reduce((s, v) => s + v, 0);
  const yearExpensesUSD = monthlyExpensesUSD.reduce((s, v) => s + v, 0);
  const yearNetSYP      = yearIncomeSYP - yearExpensesSYP;
  const yearNetUSD      = yearIncomeUSD - yearExpensesUSD;
          ...
          <>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6">
              ...
              <div className="bg-white rounded-2xl border-2 border-emerald-100 p-4 text-center">
                ...
                <p className="text-base font-bold text-emerald-600">${(yearIncomeUSD / 1000).toFixed(0)}k</p>
                <p className="text-sm font-bold text-emerald-500">{(yearIncomeSYP / 1000).toFixed(0)}k ل.س</p>
              </div>
              <div className="bg-white rounded-2xl border-2 border-red-100 p-4 text-center">
                ...
                <p className="text-base font-bold text-red-500">${(yearExpensesUSD / 1000).toFixed(0)}k</p>
                <p className="text-sm font-bold text-red-400">{(yearExpensesSYP / 1000).toFixed(0)}k ل.س</p>
              </div>
              <div className={`bg-white rounded-2xl border-2 p-4 text-center ${yearNetUSD >= 0 && yearNetSYP >= 0 ? "border-violet-100" : "border-red-200"}`}>
                <p className="text-xs font-semibold text-gray-400 mb-1">صافي الربح</p>
                <p className={`text-base font-bold ${yearNetUSD >= 0 ? "text-violet-600" : "text-red-600"}`}>
                  {yearNetUSD < 0 ? "-" : ""}${(Math.abs(yearNetUSD) / 1000).toFixed(0)}k
                </p>
                <p className={`text-sm font-bold ${yearNetSYP >= 0 ? "text-violet-500" : "text-red-500"}`}>
                  {yearNetSYP < 0 ? "-" : ""}{(Math.abs(yearNetSYP) / 1000).toFixed(0)}k ل.س
                </p>
              </div>
            </div>
            ...
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 mb-4">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-sm font-bold text-gray-700">الإيرادات مقابل المصاريف شهرياً (ل.س)</h3>
              ...
              <ComboChart income={monthlyIncomeSYP} expenses={monthlyExpensesSYP} />
            </div>

            {/* مخطط المقارنة (USD) */}
            {monthlyIncomeUSD.some((v) => v > 0) || monthlyExpensesUSD.some((v) => v > 0) ? (
              <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 mb-4">
                <div className="flex items-center justify-between mb-4">
                ...
                <ComboChart income={monthlyIncomeUSD} expenses={monthlyExpensesUSD} />
              </div>
            ) : null}
            ...
            {/* مخطط صافي الربح (ل.س) */}
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
              <h3 className="text-sm font-bold text-gray-700 mb-4">صافي الربح الشهري (ل.س)</h3>
              <div className="flex items-end gap-0.5 h-36">
                {monthlyIncomeSYP.map((inc, i) => {
                  const net = inc - monthlyExpensesSYP[i];
                  const maxAbs = Math.max(...monthlyIncomeSYP.map((v, j) => Math.abs(v - monthlyExpensesSYP[j])), 1);
